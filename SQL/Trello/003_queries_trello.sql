...
FROM 
  [Users] 
WHERE 
  Email = 'james85@booth-daniels.net';
...
FROM 
  ...
  OR (
    OwnerTypeValue = 'BOARD' 
    AND cwbw.BoardId = mmb.OwnerId
  ) 
  OR (
    OwnerTypeValue = 'WORKSPACE' 
...
WITH FieldValueCast AS (
  SELECT 
    ...
    fvl.CustomFieldId, 
    dtt.DataTypeValue, 
    CASE WHEN dtt.DataTypeValue = 'DROPDOWN' 
    AND ISNUMERIC(fvl.FieldValue) = 1 THEN CAST(fvl.FieldValue AS INT) ELSE NULL END AS ItemId, 
    fvl.FieldValue 
  FROM 
    FieldValue fvl 
...
FROM 
  ...
  JOIN Board brd ON brd.Id = stg.BoardId 
  JOIN CustomField ctf ON ctf.BoardId = brd.Id 
  LEFT JOIN FieldValueCast fvc ON fvc.CardId = crd.Id 
  AND fvc.CustomFieldId = ctf.Id 
