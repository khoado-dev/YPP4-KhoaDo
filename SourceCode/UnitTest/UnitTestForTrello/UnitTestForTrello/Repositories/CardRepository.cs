    ...
    public class CardRepository : ICardRepository
    {
        ...
        public IEnumerable<CardActivityDTO> GetActivitiesByCardId(int cardId)
        {
            ...
            WHERE 
              ...
              AND atv.OwnerId = @CardId;
            ";
            return _con.Query<CardActivityDTO>(sql, new 
            { 
                CardId = cardId,
        ...
        public IEnumerable<CardCustomFieldValueDTO> GetCustomFieldValuesByCardId(int cardId)
        {
            ...
            WITH FieldValueCast AS (
              SELECT 
                ...
                fvl.CustomFieldId, 
                dtt.DataTypeValue, 
                CASE WHEN dtt.DataTypeValue = @DataType 
                AND FieldValue GLOB '[0-9]*' -- Check if value contains only digits before casting to INTEGER
                THEN CAST(fvl.FieldValue AS INTEGER) ELSE NULL END AS ItemId, 
                fvl.FieldValue 
              FROM 
                FieldValue fvl 
            ...
            ORDER BY 
              ctf.Position;
            ";
            return _con.Query<CardCustomFieldValueDTO>(sql, new 
            { 
                CardId = cardId,
                DataType = DataType.DROPDOWN.ToString()
            });
        }
    }
}
